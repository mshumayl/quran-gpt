...
const VerseCard: FC<VerseCardProps> = ({ surah, verse, isDetailed }) => {
  ...
  return ((dbFetch.data) ? (
    <div className="bg-slate-200 m-10 p-10 border border-dashed border-slate-400 rounded-xl w-full flex flex-col text-center shadow-xl">
      ...
      ? (
      <div className="w-max sm:w-max self-center px-5 sm:px-10 flex flex-row content-evenly align-middle justify-center gap-5 items-baseline mb-5 bg-slate-300 rounded-full border border-dashed border-slate-400">
      ...
      <div className="font-zilla-slab-italic" key={`surahverse_${surah}_${verse}`}>— {dbFetch.data?.surahName}, {verse} —</div>
      <br></br>
